...
//*ürünleri ekrana rendarlayacak fonksiyon
...
//*ekle butonuna tıklanınca eklenecek ürünün idsine erişme
  const productID = parseInt(event.target.dataset.id);
  //*dizinin içerinden tıkladığımız id li elamanı bulmak için find methodu kullanıldı
  const product = products.find((product) => product.id === productID);

  if (product) {
    //*sepete tıkladığımızda ürün önceden var mı yok mu kontrol ederiz 
    const exixtingItem = cart.find((item) => item.id === productID);
    //*sepette ürün varsa bu ürünün miktarını 1 artırırız
    if (exixtingItem) {
      exixtingItem.quantity++;
    } else {
        //*ürün sepette yoksa yeni ürün ekleme şeklinde ekleriz
      const cartItem = {
        id: product.id,
        title: product.title,
        price: product.price,
      ...
      //*yeni oluşturduğumuz ürünücart dizisine ekleriz
      cart.push(cartItem);
    }
...
//*cart dizisinden ve localstorageden silmek itediğimiz ürünü sildik ve sayfayı güncelledik

function removeFromCart(event) {
  const productID = parseInt(event.target.dataset.id);
  console.log(cart);
  //*cart dizisinden silemk istediğimiz ürünü idsine göre sildik
  cart = cart.filter((item) => item.id !== productID);
  //*localstorageyi güncelledikk
  saveToLocalStorage();
  //*sayfayı güncelledik
  renderCartItems();
  //*sepetteki toplam fiyatı güncelledik 
  calculateCartTotal();
  //*toplam  miktarı güncelledik
  updateCartIcon();
}

//*imputun içindeki miktar değişince çalışacak fonksiyon
...
//*localstorageye veri eklemek için kullandık
...
function renderCartItems() {
  ...
  updateCartIcon();
  //*sepetteki yoplam fiyatı hesaplar 
}
function calculateCartTotal() {
    //*reduce iki depğer ister,bunlardan birincisi içinde yapacağımız işlem ,ikincisi başlangıç değeri
  const total = cart.reduce((sum, item) => sum + item.price * item.quantity, 0);
  cartTotalElement.textContent = `Total: $${total.toFixed(2)}`;
}
//*sayfa  carthtml sayfasındaysa rendercartitems fonk. çalışır
if (window.location.pathname.includes("cart.html")) {
  renderCartItems();
  calculateCartTotal();
} else {
    //*sayfa intexhtml sayfasındaysa renderproducts fonksiyonu çalışacak 
  renderProducts();
}
